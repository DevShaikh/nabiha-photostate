...
const AddItemForm = () => {
  ...
  const [bindingRate, setBindingRate] = useState(0);
  ...
  const bindingRateChange = (e) => setBindingRate(parseFloat(e.target.value));
  ...
  const record = {
    ...
    bill: (() => {
      if (binding !== "No") {
        return parseFloat(pages * rate * quantity + bindingRate);
      } else {
